...
원시 자료형과 달리 객체는 다양한 데이터를 담을 수 있고 키(key)와 값(value)로 구성된 프로퍼티를 여러개 넣을 수 있다.
...
생성자 함수 사용 시  반드시 `new` 키워드와 같이 호출해야한다.
...
두번째 인자에 객체의 프로퍼티 정보를 입력하면 해당 프로퍼티를 가진 객체가 생성된다.
...
ES6부터 사용할수 있게 된 문법으로 `class` 키워드로 다른 언어의 클래스와 유사한 객체를 생성할 수 있다.
...
객체의 프로퍼티는 키(key)와 값(value)로 구성되며 각 프로퍼티는 쉼표로 구분한다. 키와 값에 사용할 수 있는 값은 다음과 같다.
...
.은 일반적인 키를 선언하거나 해당하는 키로 접근하기위해 사용하고 []는 접근하려는 키가 문자열로 평가되어야 하거나 계산이 필요할 때 사용한다.
...
`delete` 연산자를 사용하여 프로퍼티를 삭제할 수 있다. 단, 객체 자체를 삭제하는것은 불가능하다.
